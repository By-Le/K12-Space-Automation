        ...
        <table class="task-table">
          ...
          <tbody>
            ...
            >
              ...
              <td><span :class="['status', task.status]" :title="taskRunAfterText(task)">{{ taskStatusText(task) }}</span></td>
              <td>
        ...
        <section class="panel modal-card settings-modal" role="dialog" aria-modal="true" aria-labelledby="settings-title">
          ...
          <div class="modal-body settings-body">
            <section class="settings-section">
              ...
              <label class="field">
                <span>K12 Workspace ID（一行一个或逗号分隔；多个时按顺序逐个执行）</span>
              ...
              <div class="switch-grid">
                <label class="switch-card">
                  ...
                  <span>
                    ...
                    <small>多个 workspace 时，每个邮箱只登录一次，并在同一任务内逐个完成 K12、入库和 JSON 写出。</small>
                  </span>
                </label>
                <label class="switch-card">
              ...
              <div class="config-grid">
                <label v-for="index in openaiProxyInputCount" :key="index" class="field">
                  <span>OpenAI 代理 {{ index }}</span>
                  <input v-model="form.openaiProxyUrls[index - 1]" placeholder="留空/direct 为直连, 或 http://127.0.0.1:7897" />
                </label>
              </div>
            </section>
          ...
          <div class="modal-body">
            <div class="result-grid">
              <div class="mini-result">
                <span>状态</span>
                <strong :title="taskRunAfterText(selectedTask)">{{ taskStatusText(selectedTask) }}</strong>
              </div>
              <div class="mini-result">
                <span>邮箱</span>
...
const openaiProxyInputCount = computed(() => Math.max(1, Math.min(10, Number(form.taskConcurrency) || 1)));
...
watch(() => form.taskConcurrency, () => {
  ensureOpenAIProxyUrlSlots();
}, {immediate: true});
...
function normalizeOpenAIProxyUrlsForForm(value: unknown, minCount = openaiProxyInputCount.value): string[] {
  const values = Array.isArray(value)
    ? value.map((item) => String(item ?? "").trim()).slice(0, 10)
    : [];
  if (!values.length) values.push("");
  while (values.length < Math.max(1, Math.min(10, minCount))) values.push("");
  return values.slice(0, 10);
}

function ensureOpenAIProxyUrlSlots() {
  form.openaiProxyUrls = normalizeOpenAIProxyUrlsForForm(form.openaiProxyUrls, openaiProxyInputCount.value);
  form.defaultProxyUrl = form.openaiProxyUrls[0] || "";
}
...
async function loadConfig() {
  ...
  const openaiProxyUrls = normalizeOpenAIProxyUrlsForForm(
    Array.isArray(config.openaiProxyUrls) ? config.openaiProxyUrls : [config.defaultProxyUrl || ""],
    config.taskConcurrency || 1,
  );
  Object.assign(form, {
    defaultProxyUrl: openaiProxyUrls[0] || config.defaultProxyUrl || "",
    openaiProxyUrls,
...
async function saveConfig() {
  ...
  try {
    ...
    const openaiProxyUrls = form.openaiProxyUrls.map((item) => String(item ?? "").trim()).slice(0, 10);
    const payload = {
      ...form,
...
async function splitSelectedEmails() {
  ...
  const count = Math.max(1, Math.min(50, Number(splitAliasCount.value) || 5));
...
function taskRunAfterText(task: TaskItem | null) {
  if (!task?.runAfter) return "";
  const date = new Date(task.runAfter);
  if (Number.isNaN(date.getTime())) return task.runAfter;
  return `计划启动: ${date.toLocaleString()}`;
}

function taskStatusText(task: TaskItem) {
  if (task.status === "queued" && isFutureTime(task.runAfter)) {
    return task.autoRetryCount ? `限流冷却 ${task.autoRetryCount}` : "冷却中";
  }
